...
BOOL Uart_OpenDevice()
{
	...
	DbgPrint(">>%d#设备打开...%s",UartIndex,UartDevIsOpened?"成功":"失败");
...
BOOL Uart_Write()
{
	...
	DbgPrint("Uart_Write %dB %s.用时%.3fS",OutLen,RetVal?"succ":"failure",UseT/1000);
...
BOOL Uart_Read()
{
	...
	DbgPrint("CH347Uart_Read %dB %s.用时%.3fS.",InLen,RetVal?"succ":"failure",UseT/1000);
...
DWORD WINAPI UartTxFileThread(LPVOID lpParameter)
{
	...
	mOpenFile.lpstrTitle        = "选择待写入数据文件";
